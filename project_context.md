...
- **Tests:** 277/277 passing (`pytest`), `ruff check src tests` clean.
...
- **In Progress:** World Fundamentals Map — **the whole planned presentation is shipped (18, P0, P1, 19, P2, 20, 21, P3, P4)**. Post-gate options in order of value: 22 (BIS DSR for Tier-3 → the last 15 empty cells + credit-bust rule on the full basket), 25 (`jurisdiction_tier` export for the börsdata radar / `/deep-dive` pre-triage), 24 (IMTS bilateral trade spike → named trade spillovers + arcs), 23 (PWT/ECI tier-C extras). Gate 2026-09-14.
