...
- **Tests:** 234/234 passing (`pytest`), `ruff check src tests` clean.
...
- **In Progress:** World Fundamentals Map — next: 19 (remaining 12 indicators incl. WGI + world shares), P2 (purpose-view selector + weights + captions, Pareto, coverage confidence). MVP gate 2026-09-14. Known nit: `bind="query-params"` updates the URL on widget interaction but not on the callback's programmatic `session_state` write (map click) — cosmetic, revisit in P1.
