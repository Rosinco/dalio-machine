...
- **Tests:** 280/280 passing (`pytest`), `ruff check src tests` clean.
...
- **In Progress:** World Fundamentals Map — **all planned slices shipped except two post-gate spikes (18, P0, P1, 19, P2, 20, 21, P3, P4, 22, 25 done)**. Remaining: 24 (IMTS bilateral trade spike → named trade spillovers + arcs), 23 (PWT/ECI tier-C extras). Coverage 328/330 (only SA has no BIS DSR; RU reported). `data/snapshots/jurisdiction_tier.csv` is the §0.2 pre-triage input (A ≥ 60 · B ≥ 40 · C): JP/CH/DE/KR/EU = A; SE/NL = B top. Gate 2026-09-14.
...
| 23 · 24 | PWT/ECI static files (tier C) · IMTS bilateral-trade spike (`IMF.STA,IMTS`) → named trade spillovers + arcs panel |
