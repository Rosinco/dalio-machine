...
- **Module Structure:**
  ...
  - `src/dalio/data_sources/` — one adapter per provider: `fred.py`, `bis.py`, `worldbank.py` (WDI source 2 + WGI source 3, paginated multi-country, never `mrv`), `imf_datamapper.py` (WEO history + projections), `imf_imts.py` (bilateral goods trade, one SDMX call per flow, partner-suffixed indicators), `oec.py` (Economic Complexity Index)
...
- **Tests:** 310/310 passing (`pytest`), `ruff check src tests` clean.
...
- **Live cycles (FRED/BIS refreshed 2026-08-24):** US LT Bubble 33 % · ST Reflation 29 % — CN LT Top (peak debt service) 47 % · ST Expansion 42 % — EU LT Reflation/financial repression 41 % · ST Inflationary peak 33 % — SE LT insufficient · ST Expansion 42 % — IN LT Transition (Bubble ↔ Debt outpaces) 26 % · ST Transition 30 % — BR LT Top 42 % · ST Reflation 37 % — UK/JP both "insufficient data" (documented FRED gaps: JP CPI mirror discontinued, UK CPI ~1 y stale). Snapshot `cycle` block carries these.
- **In Progress:** nothing — the World Fundamentals Map plan is fully shipped (slices 18, P0–P4, 19–25). Coverage 350/352. `data/snapshots/jurisdiction_tier.csv` is the §0.2 pre-triage input (A ≥ 60 · B ≥ 40 · C): JP 73.5 · CH 70.9 · DE 69.5 · KR 69.4 · EU 62.5 = A; SE 59.8 · NL 59.6 · AU 54.4 = B top. Open: moonshot-view weights (resource-rent countries top it by construction), `bind="query-params"` nit, push of the public repo.
