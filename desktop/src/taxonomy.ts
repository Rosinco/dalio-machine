import type { BusinessDocument, BusinessIndex } from './business';
import { expectedClassification, validateCatalogue, type ListingCatalogue, type BranchCounts } from './listingCatalogue';
...
export type Classification = { company_id: string; source_sector_id: string | null; source_branch_id: string | null; sector_id: string | null; branch_id: string | null; status: 'source' | 'corrected' | 'aligned' | 'needs_review' | 'sector_mismatch' | 'unclassified'; correction: Correction | null };
export type SharedStudy = { id: string; path: string; branch_ids: string[]; overview: FileRecord; deep_dives: DiveRecord[] };
export type Taxonomy = {
  catalogue?: ListingCatalogue; version: number; as_of: string; exported_at: string; business_sha256: string | null; classification_as_of: string | null;
...
export const companyBranch = (company: {id: string; branch_id: string | null}, taxonomy: Taxonomy | null) => taxonomy?.classifications[company.id] ? taxonomy.classifications[company.id].branch_id : company.branch_id;
...
export function findBranches(taxonomy: Taxonomy, query: string, sector: string, coverage: string, index: BusinessIndex | null, counts?: BranchCounts) {
...
export function validateTaxonomy(raw: any, business: BusinessDocument | null, businessHash: string | null): asserts raw is Taxonomy {
  ...
  check([1, 2].includes(raw?.version) && date(raw.as_of), 'Unsupported taxonomy version or inventory date.');
  ...
  if (raw.version === 1) check(raw.catalogue === undefined, 'A v1 taxonomy cannot contain an unvalidated company catalogue.');
  else validateCatalogue(raw.catalogue, business, raw.as_of);
  const originals = raw.version === 2 ? raw.catalogue.listings : business?.companies ?? {};
  check(raw.business_sha256 === businessHash && raw.classification_as_of === (raw.version === 2 ? raw.catalogue.as_of : business?.as_of ?? null), 'Taxonomy does not match the selected business document.');
  const assignments = objects(raw.classifications, raw.version === 2 ? 100000 : 300);
  check(assignments.length === Object.keys(originals).length, 'Taxonomy company coverage does not match the selected directory.');
  for (const [key, c] of assignments) {
    const original = originals[key];
    check(obj(c) && id(key) && c.company_id === key && original && c.source_sector_id === original.sector_id && c.source_branch_id === original.branch_id, 'Taxonomy original classification does not match its source document.');
    if (raw.version === 1) check(raw.branches[c.source_branch_id]?.sector_id === c.source_sector_id);
    const r = c.correction;
    if (r !== null) {
      check(obj(r) && r.company_id === key && id(r.expected_branch_id) && id(r.expected_sector_id) && id(r.branch_id) && raw.branches[r.branch_id] && text(r.reason) && r.reason.trim() && text(r.source) && r.source.trim() && date(r.reviewed_at) && r.reviewed_at <= raw.as_of, 'Invalid reviewed taxonomy correction.');
      if (raw.version === 1) check(raw.branches[r.expected_branch_id]?.sector_id === r.expected_sector_id);
    }
    const expected = expectedClassification(c.source_sector_id, c.source_branch_id, r, raw.branches);
    check(c.status === expected.status && c.branch_id === expected.branch_id && c.sector_id === expected.sector_id, 'Taxonomy effective classification disagrees with its correction.');
  }
}
