...
export type CompanyCoverage = { annual: Coverage; quarterly: Coverage; withheld: number; currencies: string[]; sha256: string };
...
export type FinancialCompany = { id: string; annual: SourcedReport[]; quarterly: SourcedReport[]; withheld: WithheldReport[] };
...
export function validateFinancialIndex(v: any, taxonomy: string): asserts v is FinancialIndex {
  check(v?.format === 'macro-atlas-financials' && v.version === 1 && hash(v.id) && hash(taxonomy) && v.taxonomy_sha256 === taxonomy, 'Financial history belongs to a different company directory or uses an unsupported format.');
