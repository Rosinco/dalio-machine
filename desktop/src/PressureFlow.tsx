...
export default function PressureFlow({ pressure }: { pressure: Pressure }) {
  const nodes: Node[] = [{ id: 'constraint', position: { x: 10, y: 55 }, data: { label: pressure.constraint }, style: { width: 165, background: '#e6efe7', borderColor: '#8aab97', fontSize: 11 } },
  ...
  const edges: Edge[] = pressure.forced_options.map((_, i) => ({ id: `edge-${i}`, source: 'constraint', target: `option-${i}`, style: { stroke: '#81a28d' } }));
