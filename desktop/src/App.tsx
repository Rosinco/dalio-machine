...
import { atYear, categories, finite, format, latestTrade, missingColor, palette, quintile, sourceUrl, tradeSlices } from './model';
...
export default function App() {
  ...
  const historyRange = useMemo(() => {
    const values = Object.values(histories ?? {}).flatMap(h => { const p = atYear(h[metric], year); return p && finite(p.value) ? [p.value] : []; });
    return values.length ? [Math.min(...values), Math.max(...values)] : [0, 1];
  }, [histories, metric, year]);
  const paint: Paint = useMemo(() => {
    ...
    countries.filter(([, c]) => c.on_map).forEach(([k, c]) => {
      if (mode === 'fundamentals') {
        ...
        out[k] = { color: q === null ? missingColor : palette[q], label: q === null ? 'Not enough data' : `${categories[category].short} · ${bands[q]} band · ${format(score, 0)}/100` };
      } else if (mode === 'history') {
        const value = atYear(histories?.[k]?.[metric], year)?.value;
        const q = finite(value) ? Math.min(4, Math.floor(5 * (value - historyRange[0]) / (historyRange[1] - historyRange[0] || 1))) : null;
        out[k] = { color: q === null ? missingColor : palette[q], label: finite(value) ? `${format(value)} ${metric === 'gdp_growth_fwd5' ? '% annual growth' : meta?.unit ?? ''} · ${year}` : `No historical observation for ${year}` };
      } else {
        const value = rows.find(r => r.partner === k)?.x_share;
        out[k] = { color: k === code ? '#c48d48' : finite(value) ? palette[value >= 20 ? 4 : value >= 10 ? 3 : value >= 5 ? 2 : value >= 1 ? 1 : 0] : missingColor, label: k === code ? 'Selected exporter' : finite(value) ? `${format(value)}% of ${country?.name}'s goods exports · ${rows[0]?.year}` : 'No partner value in this release' };
      }
    });
    return out;
  }, [countries, mode, category, histories, metric, year, historyRange, meta, rows, code, country]);
    ...
    <div className="workspace">
      ...
      <main className="map-panel">
        ...
        <div className="map-bottom">
          ...
          <div className="legend"><div><strong>{mode === 'fundamentals' ? 'Relative fundamentals' : mode === 'history' ? `${meta?.unit ?? ''} · ${year}` : 'Share of goods exports'}</strong><span>{mode === 'fundamentals' ? 'Higher score = stronger on this category' : mode === 'history' ? 'Raw values · colour does not indicate risk' : `Partner countries · ${rows[0]?.year ?? 'no data'}`}</span></div><div className="legend-scale">{palette.map((color, i) => <div key={color}><i style={{ background: color }} /><span>{mode === 'fundamentals' ? bands[i] : mode === 'trade' ? ['<1%', '1–5%', '5–10%', '10–20%', '20%+'][i] : format(historyRange[0] + i * (historyRange[1] - historyRange[0]) / 5, 1)}</span></div>)}</div><div className="no-data"><i />No data</div></div>
          <div className="map-footnote"><Info size={12} />{mode === 'fundamentals' ? 'Scores use the saved comparison group of 21 countries. The euro area is shown separately.' : mode === 'history' ? historyError || (!histories ? 'Loading saved annual histories…' : 'Latest saved vintage. Missing years stay blank. This is not a “known at the time” replay.') : 'The euro-area partner aggregate is excluded to avoid overlap with its member countries.'}</div>
        </div>
      </main>
      <aside className="sidebar" aria-label="Country details" data-country={code} data-ready={detail.ready}>
        {!country ? <div className="uncovered"><span className="country-badge">{code}</span><h2>{unknownName || code}</h2><p>This country has no data in the current release.</p><p>The map is global; research coverage grows as countries are added to Dalio.</p><button className="primary" onClick={() => selectCountry('SE')}>Explore Sweden <ArrowRight size={15} /></button></div> : <>
          ...
          <div className="sidebar-content">
            {tab === 'overview' && <>
              {mode !== 'history' && <section><div className="section-title"><h3>Fundamentals at a glance</h3><span className="micro">0–100</span></div><p className="section-note">Category scores · snapshot {index.as_of}</p><Radar country={country} comparison={other.country} index={index} />{other.country && <div className="chart-key"><span><i />{country.name}</span><span><i className="gold" />{other.country.name}</span></div>}
                <div className="category-list">{index.categories.map(k => { const s = country.categories[k]; const q = quintile(s?.score); return <button key={k} className={category === k && mode === 'fundamentals' ? 'selected' : ''} onClick={() => { setCategory(k); setMode('fundamentals'); }}><span>{categories[k].label}<small>{s?.n_available ?? 0}/{s?.n_total ?? 0} indicators</small></span><div className="mini-track"><i style={{ width: `${s?.score ?? 0}%`, background: q === null ? missingColor : palette[q] }} /></div><strong>{finite(s?.score) ? format(s.score, 0) : '—'}</strong></button>; })}</div>
              </section>}
              <section><div className="section-title"><h3>Through time</h3><button className="icon-button" aria-label="Export selected history as CSV" disabled={!detail.ready} onClick={exportHistory}><ArrowDownToLine size={15} /></button></div><select className="metric-select" aria-label="Chart indicator" value={metric} onChange={e => setMetric(e.target.value)}>{index.indicators.map(i => <option key={i.name} value={i.name}>{i.name === 'gdp_growth_fwd5' ? 'GDP growth · annual history & forecast' : i.label}</option>)}</select>
                ...
                <p className="chart-caption">Solid: historical observations · dashed: forecasts. {metric === 'gdp_growth_fwd5' && 'The headline is a five-year outlook; the line shows annual growth. '}History uses the saved vintage.</p>
              </section>
            ...
            {tab === 'trade' && <>
              <section><div className="section-title"><h3>Goods export destinations</h3><span className="micro">{rows[0]?.year ?? 'No data'}</span></div><p className="section-note">Top five country partners and all other destinations.</p>{slices.length ? <Chart height={245} label={`${country.name} goods exports by destination`} option={{ color: ['#245f51', '#4f8b73', '#8aaf91', '#c4b180', '#d9cfae', '#e9ebe4'], tooltip: { trigger: 'item', formatter: '{b}: {c}%' }, series: [{ type: 'pie', radius: ['52%', '74%'], center: ['50%', '49%'], itemStyle: { borderColor: '#fafbf7', borderWidth: 3 }, label: { show: false }, data: slices.map(s => ({ ...s, value: Number(s.value.toFixed(2)) })) }], graphic: [{ type: 'text', left: 'center', top: '44%', style: { text: 'GOODS\nEXPORTS', textAlign: 'center', fill: '#68776a', font: '11px Segoe UI', lineHeight: 18 } }] }} /> : <div className="empty">No compatible trade breakdown in this release.</div>}
                <div className="trade-partners">{slices.map((s, i) => <button key={s.name} disabled={!s.code} onClick={() => selectCountry(s.code)}><i style={{ background: ['#245f51', '#4f8b73', '#8aaf91', '#c4b180', '#d9cfae', '#e9ebe4'][i] }} /><span>{s.name}</span><strong>{format(s.value)}%</strong>{s.code && <ArrowRight size={13} />}</button>)}</div>
