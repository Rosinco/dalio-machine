...
import { ArrowRight, BookOpen, Building2, Calculator, ChevronRight, Globe2, Info, Layers3, ListTree, Table2, TrendingUp, Trees } from 'lucide-react';
...
import ResearchGaugeWorkspace from './ResearchGaugeWorkspace';
import CompanyListWorkspace from './CompanyListWorkspace';
import ResearchGaugeCard from './ResearchGaugeCard';
import { useResearchGauge } from './useResearchGauge';
...
type View = 'browse' | 'overview' | 'companies' | 'compare' | 'financials' | 'peers' | 'context' | 'research' | 'valuation' | 'screen' | 'lists';
const sectorViews = [{ id: 'lists', label: 'Branch company lists', short: 'Lists', icon: Table2 }, { id: 'screen', label: 'Branch research screen', short: 'Screen', icon: ListTree }, { id: 'browse', label: 'Browse sectors and branches', short: 'Browse', icon: ListTree }, { id: 'overview', label: 'Branch overview', short: 'Branch', icon: Trees }, { id: 'companies', label: 'Branch companies', short: 'Companies', icon: Building2 }, { id: 'compare', label: 'Branch comparison', short: 'Compare', icon: TrendingUp }, { id: 'context', label: 'Branch macro context', short: 'Context', icon: Globe2 }, { id: 'research', label: 'Branch research', short: 'Research', icon: BookOpen }] as const;
const companyViews = [{ id: 'lists', label: 'Company lists', short: 'Lists', icon: Table2 }, { id: 'financials', label: 'Company financials', short: 'Profile', icon: TrendingUp }, { id: 'screen', label: 'Universe research screen', short: 'Screen', icon: ListTree }, { id: 'valuation', label: 'Company valuation', short: 'Value', icon: Calculator }, { id: 'peers', label: 'Company peers', short: 'Peers', icon: Building2 }, { id: 'context', label: 'Company macro context', short: 'Context', icon: Globe2 }, { id: 'research', label: 'Company research', short: 'Research', icon: BookOpen }] as const;
...
export default function BusinessWorkspace({ observatory, index, taxonomy, financial, financialReady, financialError, ready, error, macro, release, code, selectedName, companyId, branchId, onBranch, onCountry, onCompany, onMacro, onSector, onLibrary }: {
  ...
  const gauge = useResearchGauge(financial, release.taxonomy_sha256, ready && financialReady && !error && !financialError && (view === 'screen' || view === 'lists' || observatory === 'companies' && view === 'financials'));
  const gaugeRow = useMemo(() => gauge.data?.rows.find(row => row.id === companyId) ?? null, [gauge.data, companyId]);
  ...
  // Both resources have been validated against this selected release. The
  // directory inventories archived deep dives beyond the five prose profiles.
  const valuationSources = useMemo(() => [...index?.sources ?? [], ...taxonomy ? [
    ...Object.values(taxonomy.branches).flatMap(b => b.deep_dives),
    ...Object.values(taxonomy.shared_studies).flatMap(s => s.deep_dives),
    ...taxonomy.unmapped_deep_dives,
  ].flatMap(d => d.documents) : []], [index, taxonomy]);
  ...
  return <div className="workspace business-workspace" data-observatory={observatory} data-listing-count={entries.length} data-financial-count={financial?.summary.with_reports ?? 0}>
    ...
    {view === 'lists' ? <CompanyListWorkspace financial={financial} taxonomySha256={release.taxonomy_sha256} key={`${release.id}:${financial?.id}:${observatory}:${observatory === 'sectors' ? activeBranchId : 'all'}`} data={gauge.data} ready={gauge.ready && ready && financialReady} error={error || financialError || gauge.error} initialBranchId={observatory === 'sectors' ? activeBranchId : undefined} onCompany={id => { onCompany(id); setView('financials'); }} onBack={() => setView(observatory === 'companies' ? 'financials' : 'overview')} /> : view === 'screen' ? <ResearchGaugeWorkspace key={`${release.id}:${financial?.id}:${observatory}:${observatory === 'sectors' ? activeBranchId : 'all'}`} data={gauge.data} ready={gauge.ready && ready && financialReady} error={error || financialError || gauge.error} initialBranchId={observatory === 'sectors' ? activeBranchId : undefined} onCompany={id => { onCompany(id); setView('financials'); }} onBack={() => setView(observatory === 'companies' ? 'financials' : 'overview')} /> : observatory === 'companies' && view === 'valuation' && entry && ready && !error && financialReady && history.ready ? <ValuationWorkspace key={`${release.id}:${financial?.id}:${entry.id}`} entry={entry} classification={taxonomy?.classifications[entry.id]} financial={financial} history={history.data} release={release} researched={researchedStudyFor(entry, valuationSources)} onProfile={() => setView('financials')} /> : observatory === 'sectors' && view === 'compare' ? <BranchExplorer key={`${release.id}:${financial?.id}:${activeBranchId}`} entries={branchEntries} financial={financial} financialReady={financialReady} financialError={financialError} release={release} taxonomy={taxonomy} branchId={activeBranchId} branchName={branchName} onBranch={onBranch} onBrowse={browse} onCompany={onCompany} /> : <>
