import FinancialLibrary, { type FinancialLibraryProps } from './FinancialLibrary';
...
export default function ResearchLibrary({ releases, active, unreadable, onUse, onImported, onClose, financial, financialReady, financialError, onFinancialImported }: FinancialLibraryProps & {
  ...
  return <div className="modal-backdrop" onClick={onClose}>
    <div className="modal research-library" role="dialog" aria-modal="true" aria-labelledby="library-title" onClick={e => e.stopPropagation()}>
      ...
      <p>Import an Atlas research file to update macro data and any included sector and company research. Earlier releases stay available. Everything here works offline.</p>
      ...
      <FinancialLibrary financial={financial} financialReady={financialReady} financialError={financialError} onFinancialImported={onFinancialImported} />
      ...
      <p className="chart-caption">Macro Atlas 0.8.0 · Country boundaries: Natural Earth, public domain. Company coverage follows saved Börsdata listing countries. Verified physical assets and new branch forecasts are future research stages.</p>
    </div>
  </div>;
}

function ReleaseDescription({ release }: { release: ResearchRelease }) {
  return <p className="release-description">{release.country_count} economies · {release.indicator_count} indicators<br />{release.liquidity_as_of ? `Liquidity diagnostics · ${release.liquidity_as_of}` : 'Fundamentals, history and trade'}<br />{release.business_as_of ? `${release.company_count} financial profiles · Börsdata ${release.business_as_of}` : 'No company research included'}{(release.listing_count ?? 0) > 0 && <><br />{release.listing_count!.toLocaleString('en-US')} company listings</>}<br />{release.taxonomy_as_of ? `${release.sector_count} sectors · ${release.branch_count} branches · inventory ${release.taxonomy_as_of}` : 'No full branch directory included'}</p>;
}
