...
export default function ResearchLibrary({ releases, active, unreadable, onUse, onImported, onClose }: {
  ...
  return <div className="modal-backdrop" onClick={onClose}>
    <div className="modal research-library" role="dialog" aria-modal="true" aria-labelledby="library-title" onClick={e => e.stopPropagation()}>
      ...
      <details className="library-help"><summary>How to update the research</summary><p>Export a Macro Atlas research file from the desktop tools, then choose “Import research file” here. The file contains saved fundamentals and, when included, liquidity diagnostics and Börsdata company research. Importing works without rebuilding or reinstalling Atlas.</p><p>Imported releases are copied into your local application data. “Save a copy” exports a portable research file for backup or another computer. There is no automatic online refresh.</p></details>
      <p className="chart-caption">Macro Atlas 0.3.0 · Country boundaries: Natural Earth, public domain. Company coverage shows selected listings. Verified physical assets and new branch forecasts are future research stages.</p>
    </div>
  </div>;
}

function ReleaseDescription({ release }: { release: ResearchRelease }) {
  return <p className="release-description">{release.country_count} economies · {release.indicator_count} indicators<br />{release.liquidity_as_of ? `Liquidity diagnostics · ${release.liquidity_as_of}` : 'Fundamentals, history and trade'}<br />{release.business_as_of ? `${release.company_count} companies · Börsdata ${release.business_as_of}` : 'No company research included'}</p>;
}
