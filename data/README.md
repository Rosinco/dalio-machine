...
The current 24-policy / 19-organization snapshot is
...
snapshots remain registered and exact-validated.
...
The checked, metadata-only 2025 Reserve Bank of Australia cohort is:
...
The immutable pair is
`communication_metadata_2026-09-09_82d5024c65e927e8.{json,md}`. The full
semantic manifest SHA-256 is
`00d6016fdf83288ceb28483760f1adac6415a9d18eea3e45bc5a21278f85aeab`,
and the inventory self-hash is
...
the generated JSON and Markdown file SHA-256 values are respectively
`9028b5c26f720139869360aa1ab158035ab64786f6b4c2fce9f621d841bab4c9`
and `913233696151c9b26e6dfd173aa6a74ed52d6d850111152c220943dab18b0490`.
The output measures eight exact first-party inline-HTML transcript-page
locators, eight exact external video URLs linked from those official RBA pages
and zero verified exact caption tracks against the same closed eight-event
denominator. No slide representation is claimed, and RBA MP3 audio is outside
Version 1. This metadata authorizes no content capture, rights decision or
database write. Publishing it does not move the BoE or Riksbank latest aliases.
...
The 2026-09-10 refinancing collection adds 697 observations in 31 immutable
...
| `artifacts/debt_refinancing/` | Content-addressed Eurostat/ECB source responses, native payloads, missingness ledgers and ingestion catalogues; dated staging/live audits | Durable release-bound evidence; all four artifact roles are required for readiness |
| `backups/dalio-before-refinancing-2026-09-10.sqlite3` | Verified exact database backup before the first harmonized refinancing ingestion | Recovery baseline; staging and live preserve every pre-existing row across 25 tables |
| `snapshots/refinancing_2026-09-10_651235cedddb.{json,md}` | Dated descriptive comparison from 31 audited histories | Common-year national comparison, explicit matched-denominator due share and separate EA21 securities comparator; no composite risk score |
...
- Sovereign refinancing stores 29 annual general-government partitions for
  DE/FR/IT/ES/SE and two monthly fixed-composition EA21 comparison histories.
  Each release binds exact source bytes, canonical native payload, missingness
...
- Institutional communications use a sibling ledger rather than the bounded
  ...
  tables are empty: the 24-policy catalogue covers 19 organizations and is
  ...
  RBA's closed eight-event denominator separately has eight exact first-party
  inline-HTML transcript pages and eight exact external video URLs linked from
  those official pages, while exact caption tracks remain unverified for all
  eight events. The external platform is not a first-party RBA host. No RBA
  slide representation is claimed, and its MP3 audio remains outside Version 1.
...
Refinancing refresh and offline promotion (run from the repository with the
virtual environment active):

```bash
python -m dalio.pipelines.fetch_sovereign_refinancing --db <staging.sqlite>
python scripts/audit_observatory.py --db <staging.sqlite> --json
# After staging verification and an exact backup of the working database:
python -m dalio.pipelines.fetch_sovereign_refinancing --db data/dalio.db --from-db <staging.sqlite>
python -m dalio.pipelines.build_refinancing_brief --db data/dalio.db --as-of 2026-09-10
```

The refresh defaults to fetching complete official responses; `--use-cache`
allows validated local caches. Retain the entire `data/artifacts/debt_refinancing/`
tree: absolute artifact paths are bound to releases and staging promotion
references the same files. The read-only audit re-hashes and re-parses evidence
and compares both observation stores. A disappearing formerly finite period
blocks the whole batch pending a source-contract review. Dated brief exports
go to `data/snapshots/`; a common year is required for cross-country comparison.
The due-within-one-year share uses same-country, same-year Eurostat residual
maturity totals, never QPSD or the different ECB securities denominator.
...
Selecting `--cohort rba_2025_monetary_policy_media_conferences` uses the pinned
RBA manifest and refreshes only
`review/communication_metadata_rba_2025_monetary_policy_media_conferences_latest.{json,md}`.
All cohorts use generic hash-bound immutable filenames. An unknown cohort or a
...
- Institutional communication history has not yet been collected. The 24-policy,
  ...
  caption tracks. The closed 2025 RBA cohort adds eight first-party inline-HTML
  transcript pages and eight officially linked external video URLs, but zero
  verified exact caption tracks; slides are not claimed and MP3 audio remains
  outside Version 1. The bounded Riksbank-plus-RBA package is complete at 16/16
  denominator events. These are only metadata inventories: no source authorizes
