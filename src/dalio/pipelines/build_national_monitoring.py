"""Publish an offline US/Germany/Canada monitoring comparison."""
...
from dalio.national_monitoring.acquisition import load_evidence, utc
from dalio.national_monitoring.core import COUNTRIES, build_snapshot, validate_snapshot
...
DEFAULT_ARTIFACT_ROOT = Path("data/artifacts/national_monitoring")
DEFAULT_OUTPUT_DIR = Path("data/snapshots/national_monitoring")


def load_inputs(engine, *, as_known_at, as_of, bundle_paths):
    national = load_evidence(bundle_paths=bundle_paths, as_known_at=as_known_at)
    assessment = build_assessment(load_assessment(engine, as_known_at=as_known_at, countries=list(COUNTRIES)), as_of=as_of)
    return national, assessment


def export_monitoring(*, db_path: Path, as_of=None, as_known_at=None, bundle_paths=None,
                      artifact_root=DEFAULT_ARTIFACT_ROOT, output_dir=DEFAULT_OUTPUT_DIR,
    ...
    if as_of > cutoff.date():
        raise ValueError("National assessment date is after the UTC knowledge cutoff")
    ...
    if len({Path(p).resolve() for p in paths}) != len(paths):
        raise ValueError("Duplicate National bundle paths")
    protected = {db_path.resolve(), *(Path(p).resolve() for p in paths)}
    ...
    try:
        inputs = load_inputs(engine, as_known_at=cutoff, as_of=as_of, bundle_paths=paths)
