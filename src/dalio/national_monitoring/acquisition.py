...
METHOD = "national-monitoring-v1"
COUNTRIES = ("US", "DE", "CA")
INDICATORS = ("industrial_production", "corporate_new_lending_rate", "policy_rate", "yield_10y")


def catalogue(end: date) -> list[dict]:
    from dalio.data_sources import canada_monitoring, germany_monitoring, us_monitoring

    result = [spec for module in (us_monitoring, germany_monitoring, canada_monitoring)
    ...
    if len(result) != len(expected) or {(s["country"], s["indicator"]) for s in result} != expected:
        raise ValueError("National catalogue must contain the fixed twelve country/signals")
    return result
...
def parse_input(spec: dict, bodies: dict[str, bytes]) -> dict:
    ...
    module = {"US": us_monitoring, "DE": germany_monitoring, "CA": canada_monitoring}[spec["country"]]
...
def load_bundle(path: Path) -> dict:
    ...
    if path.is_symlink():
        raise ValueError("National bundle symlink is not permitted")
    path = path.resolve()
    body = path.read_bytes()
    sha = digest(body)
    bundle = json.loads(body)
    if path.stem != sha or canonical(bundle) != body or bundle.get("method") != METHOD:
        raise ValueError("National bundle hash/contract mismatch")
    started, completed = utc(bundle["started_at"]), utc(bundle["available_at"])
    if started > completed or utc(bundle["retrieved_at"]) != completed:
        raise ValueError("National batch clock ordering is invalid")
    if bundle["history_end"] != started.date().isoformat():
        raise ValueError("National history end differs from capture date")
    specs = catalogue(started.date())
    if len(bundle["signals"]) != len(specs):
        raise ValueError("National capture must contain every fixed signal attempt")
    ...
    for item, spec in zip(bundle["signals"], specs, strict=True):
        ...
        if any(item.get(k) != v for k, v in identity.items()) or item["spec_sha256"] != digest(canonical(spec)):
            raise ValueError("National signal identity or semantic catalogue differs")
        if len(item["requests"]) != len(spec["requests"]):
            raise ValueError("National request set is incomplete")
        ...
        for record, request in zip(item["requests"], spec["requests"], strict=True):
            if any(record.get(key) != request.get(key) for key in ("role", "method", "url", "json")):
                raise ValueError("National request identity differs from official catalogue")
            requested, received = utc(record["requested_at"]), utc(record["received_at"])
            if not started <= requested <= received <= completed:
                raise ValueError("National request clock lies outside batch")
            raw = None
            if record["path"] is not None:
                raw_path = Path(record["path"])
                if not raw_path.is_absolute() or raw_path.is_symlink() or str(raw_path.resolve()) != str(raw_path):
                    raise ValueError("National artifact path must be a canonical regular file")
                raw = raw_path.read_bytes()
                if digest(raw) != record["sha256"]:
                    raise ValueError("National raw artifact hash mismatch")
                ...
                raise ValueError("National artifact hash has no retained bytes")
            if record["error"] is None:
                if (record["http_status"] != 200 or record["final_url"] != request["url"]
                        or record["redirect_history"] or raw is None):
                    raise ValueError("National successful request lacks original delivery evidence")
            elif not isinstance(record["error"], str) or not record["error"]:
                raise ValueError("National source error must be explicit")
            else:
                errors.append({"role": request["role"], "error": record["error"]})
            if request["role"] in bodies:
                raise ValueError("Duplicate National request role")
        ...
        try:
            ...
            if any(parsed.get(k) != v for k, v in identity.items()):
                raise ValueError("Parsed National identity differs from requested source")
            unavailable = spec.get("unavailable_reason")
            if unavailable:
                if (not artifacts or parsed.get("unavailable_reason") != unavailable
                        or "observations" in parsed):
                    raise ValueError("Structural gap requires verified documentation and no substituted observations")
                result["gaps"].append({**identity, "reason": "structural_gap", "error": unavailable,
                    "definition": spec.get("definition"), "available_at": completed.isoformat(),
                    "source_urls": [r["url"] for r in spec["requests"]],
                    "artifacts": [*artifacts, {"role": "acquisition_bundle", "sha256": sha, "path": str(path)}]})
                continue
            if "unavailable_reason" in parsed:
                raise ValueError("Undeclared structural gap")
            ...
            metadata = parsed.get("publisher_metadata") or {}
            native_date = metadata.get("report_updated_date")
            if native_date:
                # Date-only evidence stays date-only. With no declared native
                # timezone, UTC receipt day is a conservative eligibility rule.
                zone = ZoneInfo(metadata.get("update_timezone") or "UTC")
                earliest_day = min(utc(r["received_at"]).astimezone(zone).date() for r in item["requests"])
                if date.fromisoformat(native_date) > earliest_day:
                    raise ValueError("Publisher report date postdates source receipt day")
            canonical(parsed)
