"""Offline Nordic comparison retaining native definitions and evidence clocks."""
...
from dalio.nordic_monitoring.core import COUNTRIES, validate_snapshot
...
def _ordered(snapshot):
    ...
    if len(countries) != len(snapshot["countries"]) or set(countries) != set(COUNTRIES):
        raise ValueError("Nordic report requires four distinct country identities")
...
def _refs(refs, snapshot, numbers, *, country, prefix="") -> str:
    ...
    for ref in dict.fromkeys(refs):
        ...
        if not record or record.get("country") != country:
            raise ValueError("Unresolved or wrong-country Nordic citation")
...
def _common_intro(snapshot) -> list[str]:
    return [
        ...
        "Index levels with different bases or coverage are not directly comparable; rates do not measure credit access.",
        "",
...
def render_index(snapshot: dict) -> str:
    ...
    lines = ["# Nordic scenario monitoring", "", *_common_intro(snapshot), "",
    ...
    lines.extend(["", "Coverage is not a predictive-confidence measure. Swedish order intake is supplemental "
                  "and appears in [Sweden’s report](SE.md); it is outside the four shared topics."])
    ...
    lines.extend(["", _text(snapshot["methodology"].get("freshness_policy", "")), "",
                  ...
                  "Finland’s applicable ECB instrument is shared euro-area policy, not an independent Finnish "
...
def render_country(snapshot: dict, code: str) -> str:
    ...
    if code not in COUNTRIES:
        raise ValueError("Unknown Nordic report country")
    ...
    lines = [f"# {_text(country['name'])} — scenario monitoring", "",
             f"[Nordic comparison](index.md) · [Annual IMF reference and scenario context](context/{code}.md)", "",
